...
==== To install Kroki on default namespace
...
=== To install Kroki on kroki namespace (Optional)

[shell]
....
kubectl create ns/kroki
kubectl apply -n kroki -f docs/modules/setup/examples/k8s/
....
...
=== Optional to create an kubernetes ingress. Alternatively use loadbalancer on your existing service.

==== Sample kubernetes ingress yaml

[source,yaml]
----
include::example$k8s$kroki-ingress.yaml[]
---
